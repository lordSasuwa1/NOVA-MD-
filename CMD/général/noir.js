...
module.exports = {
  config: { name: 'noir', aliases: ['grayscale','nb'], description: 'Convertit une image en noir et blanc', usage: 'noir (citer image)', permission: 0, category: 'général' },
  async execute(ctx, db) {
    ...
    try {
      ...
      const result = await sharp(buffer).grayscale().jpeg().toBuffer()
      await sock.sendMessage(from, { image: result, caption: '🖤 Image en noir & blanc' }, { quoted: msg })
    } catch {
      await ctx.react('❌')
